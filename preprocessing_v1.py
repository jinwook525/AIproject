...
def preprocess():
    ...
    for _, row in df_ais.iterrows():
        mmsi = int(row["mmsi"])  # ✅ MMSI를 정수형으로 변환하여 key 설정
        tensor = torch.tensor(row[["turn", "speed", "accuracy", "course", "heading"]].values, dtype=torch.float32)
        processed_data[mmsi] = tensor  # ✅ 딕셔너리 key를 `mmsi`로 설정
    # 병합된 데이터 사용
    ...
    # ✅ 데이터 변환 (Pandas → PyTorch Tensor)
    processed_data = {}

    for _, row in df_ais.iterrows():
        mmsi = int(row["mmsi"])  # ✅ MMSI를 정수형으로 변환하여 key 설정
        
        # ✅ 값 가져오기 및 변환 (숫자가 아닐 경우 NaN으로 변환 후 float으로 변환)
        tensor_values = row[["turn", "speed", "accuracy", "course", "heading"]].astype(str).replace({"None": np.nan}).values
        tensor_values = pd.to_numeric(tensor_values, errors="coerce").astype(float)  # ✅ 숫자로 변환, NaN 처리
        tensor_values = np.nan_to_num(tensor_values, nan=0.0)  # ✅ NaN 값을 0으로 대체
        tensor = torch.tensor(tensor_values, dtype=torch.float32)  # ✅ PyTorch Tensor 변환

        processed_data[mmsi] = tensor  # ✅ 딕셔너리 key를 `mmsi`로 설정

    print("✅ 변환 후 `df_ais` 데이터 타입 확인:")
    print(df_ais.dtypes)
...
# ✅ PyTorch Tensor 변환 (각 MMSI별 데이터 반환)
    processed_data = {mmsi: torch.tensor(normalized_data[idx], dtype=torch.float32)
                      for idx, mmsi in enumerate(df_ais["mmsi"].values)}

    print("✅ Preprocessing 완료. 반환되는 MMSI 목록:", list(processed_data.keys()))

    return processed_data  # ✅ 최종 데이터 반환
